with target_model as (
    ...
    join (
        values
            ('qwen-plus', 0.0040::numeric, 0.0120::numeric, 0.0100::numeric, 0.0300::numeric, 1::numeric),
            ('deepseek-chat', 0.0060::numeric, 0.0180::numeric, 0.0150::numeric, 0.0400::numeric, 1::numeric)
...
inserted_version as (
    ...
    select
        (910000000000000000 + row_number() over (order by model_id))::bigint,
        model_id,
        version_no,
        input_price,
        output_price,
        ...
        billing_multiplier,
        now(),
        'Normalize MVP per-token credit rates: 1 CNY = 1000 Credits',
        now(),
        now()
    from target_model
